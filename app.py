...
REAL_GAS_FACTORS = {
    "Luft": lambda p, T: max(0.8, 1.0 - 0.0001 * p),
...
def generate_viscous_curve(pump, nu_cSt, rho):
    ...
    for Q_w, H_w, eta_w in zip(pump["Qw"], pump["Hw"], pump["eta"]):
        ...
        P_v = (rho * G * (Q_v / 3600.0) * H_v) / (1000.0 * max(eta_v, 1e-6))
        Q_vis.append(Q_v)
        H_vis.append(H_v)
...
def total_gas_from_gvf(gvf_pct, p_bar, T_celsius, gas="Luft"):
    try:
        ...
        T_K = T_celsius + 273.15
        V_oper = (gvf_pct / 100.0) * (R_BAR_L * T_K) / (p_bar * Z)
        V_N = V_oper * (p_bar / P_N_BAR) * (T_N_K / T_K)
        return V_N * 1000.0
    except:
        return 0.0
