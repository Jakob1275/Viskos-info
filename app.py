...
# Unterdrücke SciPy-Warnungen für bessere Lesbarkeit
warnings.filterwarnings("ignore", category=UserWarning)

# =========================
# Konstanten mit Fehlerbehandlung
# =========================
try:
    ...
    P_N_BAR = 1.01325  # bar
    T_N_K = 273.15  # K
    BAR_TO_M_WATER = 10.21  # m/bar
    N0_RPM_DEFAULT = 2900  # 1/min
except Exception as e:
    st.error(f"Fehler bei der Initialisierung der Konstanten: {e}")
    st.stop()

# =========================
# Medien- und Gasdaten mit Validierung
# =========================
MEDIA = {
...
REAL_GAS_FACTORS = {
    "Luft": lambda p, T: max(0.8, 1.0 - 0.0001 * p),  # Mindestwert 0.8
    "Methan": lambda p, T: max(0.7, 1.0 - 0.0002 * p),
    "CO2": lambda p, T: max(0.6, 0.9 + 0.00005 * (T - 273.15)),
}

# =========================
# Pumpendaten mit erweiterter Fehlerbehandlung
# =========================
PUMPS = [
    {
...
# =========================
# Hilfsfunktionen mit erweiterter Fehlerbehandlung
# =========================
def safe_clamp(x, a, b):
    """Sichere Version von clamp mit Fehlerbehandlung."""
    try:
...
def safe_interp(x, xp, fp):
    """Sichere Interpolation mit Fehlerbehandlung."""
    try:
        if len(xp) != len(fp):
            raise ValueError("xp und fp müssen gleiche Länge haben")
        if len(xp) < 2:
        ...
        st.warning(f"Interpolationsfehler: {e}")
        return fp[-1] if fp else 0.0

def m3h_to_lmin(m3h):
    """Sichere Umrechnung m³/h → L/min."""
    try:
        return m3h * 1000 / 60
    except:
        return 0.0
...
# =========================
# Viskositätskorrektur mit Fehlerbehandlung
# =========================
def compute_B_HI(Q_m3h, H_m, nu_cSt):
    """Berechnet die HI-Kennzahl B mit Fehlerbehandlung."""
    try:
        Q = max(Q_m3h, 1e-6)
        H = max(H_m, 1e-6)
        nu = max(nu_cSt, 1e-6)
        ...
        return 1.0  # Neutraler Wert bei Fehler

def viscosity_correction_factors(B):
    """Berechnet Korrekturfaktoren mit Fehlerbehandlung."""
    try:
        if B <= 1.0:
            return 1.0, 1.0
        ...
        return 1.0, 1.0  # Neutraler Wert bei Fehler

# =========================
# Gaslöslichkeit mit erweiterter Fehlerbehandlung
# =========================
def henry_constant(gas, T_celsius):
    """Berechnet Henry-Konstante mit Fehlerbehandlung."""
    try:
        ...
        return 1400.0  # Standardwert bei Fehler

def real_gas_factor(gas, p_bar, T_celsius):
    """Berechnet Realgasfaktor mit Fehlerbehandlung."""
    try:
        T_K = T_celsius + 273.15
...
def gas_solubility_cm3N_per_L(gas, p_bar_abs, T_celsius, y_gas=1.0):
    """Berechnet Gaslöslichkeit mit Fehlerbehandlung."""
    try:
        p = max(p_bar_abs, 1e-6)
        T_K = T_celsius + 273.15
        H = henry_constant(gas, T_celsius)
        Z = real_gas_factor(gas, p, T_celsius)
        p_partial = safe_clamp(y_gas, 0.0, 1.0) * p
        C_mol_L = p_partial / max(H, 1e-12)
        V_molar_oper = (R_BAR_L * T_K) / (p * Z)
        V_oper_L_per_L = C_mol_L * V_molar_oper
        ratio = (p / P_N_BAR) * (T_N_K / T_K)
        return V_oper_L_per_L * ratio * 1000.0
    except:
        return 0.0  # Keine Löslichkeit bei Fehler

# =========================
# Pumpenauswahl mit erweiterter Fehlerbehandlung
# =========================
def find_speed_ratio(Q_curve, H_curve, Q_req, H_req, n_min=0.5, n_max=1.2):
    """Findet Drehzahlverhältnis mit Fehlerbehandlung."""
    try:
...
def choose_best_pump(pumps, Q_req, H_req, nu_cSt, rho, allow_out_of_range=True):
    """Wählt beste Pumpe mit Fehlerbehandlung."""
    try:
        best = None
        for p in pumps:
            try:
...
def choose_best_mph_pump(pumps, Q_req, dp_req, gvf_free_pct, nu_cSt, rho_liq):
    """Wählt beste Mehrphasenpumpe mit Fehlerbehandlung."""
    try:
        best = None
        for pump in pumps:
            try:
                ...
                # Prüfung bei Nenndrehzahl
                if min(curve["Q"]) <= Q_req <= max(curve["Q"]):
                    dp_avail = safe_interp(Q_req, curve["Q"], curve["dp"])
                    if dp_avail >= dp_req:
                        P_req = safe_interp(Q_req, power_curve["Q"], power_curve["P"])
                ...
                # Prüfung mit Drehzahlanpassung
                def dp_at_ratio(n_ratio):
                    Q_base = Q_req / n_ratio
                    dp_base = safe_interp(Q_base, curve["Q"], curve["dp"])
...
# =========================
# NPSH-Berechnung mit Fehlerbehandlung
# =========================
def calculate_npsh_available(p_suction, p_vapor, H_suction_loss, rho, G):
    """Berechnet verfügbaren NPSH-Wert mit Fehlerbehandlung."""
    try:
        return (p_suction - p_vapor) / (rho * G) + H_suction_loss
    except:
        return 0.0  # Kein NPSH bei Fehler

# =========================
# Streamlit App mit erweiterter Fehlerbehandlung
# =========================
def main():
    try:
...
def run_single_phase_pump():
    """Einphasenpumpen-Auslegung mit Fehlerbehandlung."""
    try:
        ...
        if n_ratio_opt:
            ...
            saving_pct = (P_nom_at_Q - P_opt_kW) / max(P_nom_at_Q, 1e-6) * 100.0 if P_nom_at_Q > 0 else 0
        else:
            P_opt_kW = None
            saving_pct = None
        ...
        try:
            ...
            ax3.scatter([Q_water], [safe_interp(Q_water, pump["Qw"], pump["Pw"])], marker='^', s=100, label="BP (Wasser)")
            ...
            st.error(f"Fehler bei der Diagrammerstellung: {e}")
        ...
        st.error(f"Fehler in der Einphasenpumpen-Auslegung: {e}")

def run_multi_phase_pump():
    """Mehrphasenpumpen-Auslegung mit Fehlerbehandlung."""
    try:
        ...
        st.error(f"Fehler in der Mehrphasenpumpen-Auslegung: {e}")

def run_atex_selection():
    """ATEX-Auslegung mit Fehlerbehandlung."""
    try:
        ...
        with st.expander("Technische Details"):
            ...
            - **Wirkungsgradklasse:** {selected_motor.get('efficiency_class', 'N/A')}
